<template>
	<view style="padding: 20rpx 40rpx;">
		 <view class="bt_primary" style="border-radius: 20rpx;padding: 20rpx;margin-bottom: 20rpx;" v-for="item in postList">
			 ...
			 <view @click="handlerToPath(item)"  style="text-align: center;margin-top: 20rpx;padding: 5rpx 0;" class="bt_primary">点击查看原帖</view>
			<view style="text-align: center;margin-top: 20rpx;display: flex;justify-content: space-between;">
				<view @click="ignore(item.id)" class="bt_primary" style="flex: 1;text-align: center;margin-right: 10rpx;">不删除</view>
				<view @click="deletePost(item.id)" class="bt_primary" style="flex: 1;text-align: center;margin-left: 10rpx;">删除</view>
			</view>
		 </view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			ignore(id){
				uni.request({
				  url: 'http://localhost:8088/ssmDemo_war/admin/opt/refuseDelete',
				  method: 'POST',
				  data: {
				      postId: id,
				  },
			...
			getPosts(){
				uni.request({
				  url: 'http://localhost:8088/ssmDemo_war/admin/check/reportedPost',
				  method: 'GET',
				  header: {
				    'content-type': 'application/x-www-form-urlencoded',
				    'Access-Control-Allow-Origin': '*',
			...
			deletePost(id){
				console.log(id),
				uni.request({
				  url: 'http://localhost:8088/ssmDemo_war/admin/opt/deletePost',
				  method: 'POST',
				  data: {
				      postId: id,
				  },
